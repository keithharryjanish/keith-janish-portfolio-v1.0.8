...
import {projectMedia, mediaUrl} from '../lib/media-paths.js';
...
function projectCard(p) {
 ...
 return `<article class="project-card ${e(p.slug)}" data-project-tags="${e(JSON.stringify(projectTags(p)))}">
  <a class="project-visual" href="/projects/${p.slug}/" tabindex="-1" aria-hidden="true">
   <img src="${e(media.image)}" alt="${e(media.imageAlt)}" loading="lazy" decoding="async">
  ...
  <div class="project-copy">
   ...
   <div class="card-actions"><a href="/projects/${p.slug}/" class="case-link">Explore project ${arrow}</a>${projectVideo(p) ? `<a class="card-video" href="/projects/${p.slug}/#video"><span aria-hidden="true">▷</span> Watch video</a>` : ''}</div>
  </div>
 </article>`;
}
export function home() {
 ...
 <main id="main">
  ...
  <section class="section work wrap" id="projects">
   ...
   <noscript><p>Enable JavaScript to use project filters. All projects are shown below.</p></noscript><div class="project-carousel" role="region" aria-roledescription="carousel" aria-label="Projects">
    <div class="carousel-toolbar"><span class="carousel-label">EXPLORE THE WORK</span><div class="carousel-navigation"><p class="carousel-position" data-carousel-position role="status" aria-live="polite">${projects.length} projects</p><div class="carousel-buttons"><button type="button" class="carousel-button" data-carousel-prev aria-label="Previous projects" aria-controls="project-track" disabled><svg viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false"><path d="M19 12H5m0 0 6 6m-6-6 6-6"/></svg></button><button type="button" class="carousel-button" data-carousel-next aria-label="Next projects" aria-controls="project-track" disabled><svg viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false"><path d="M5 12h14m0 0-6-6m6 6-6 6"/></svg></button></div></div></div>
